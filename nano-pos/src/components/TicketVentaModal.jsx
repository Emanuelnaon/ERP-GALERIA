...
export default function TicketVentaModal({ cart, total, numVenta, localNombre, vendedorEmail, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 print:static print:block print:p-0 print:bg-transparent">
            <div className="bg-gray-900 text-gray-200 border border-gray-700 font-mono w-full max-w-sm md:max-w-md lg:max-w-xl p-6 md:p-10 shadow-2xl relative print:border-none print:shadow-none print:w-full print:max-w-[300px] print:p-0 print:bg-white print:text-black print:scale-100">
                <div className="text-center mb-6 md:mb-10">
                    {/* Título Gigante en PC, normal en impresión */}
                ...
                <div className="mb-6 md:mb-10">
                    <table className="w-full text-left text-base md:text-xl print:text-xs">
                        ...
                        <tbody>
                            {cart.map((item, index) => (
                                <tr key={index} className="border-b border-gray-800/50 print:border-none last:border-0">
                                    <td className="py-4 print:py-1 align-top pr-2">
                                        <span className="font-bold text-white print:text-black">{item.cantidad}x</span>{' '}
                                        {item.nombre}
                                        {item.talle && (
                ...
                <div className="absolute -bottom-20 md:-bottom-24 left-0 right-0 flex gap-4 print:hidden">
                    <button
                        onClick={handlePrint}
                        className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-4 md:py-6 rounded-xl flex justify-center items-center gap-3 font-bold shadow-lg text-xl md:text-2xl transition-all active:scale-95">
                        <Printer size={28} /> Imprimir
                    </button>
                    <button
                        onClick={onClose}
                        className="flex-1 bg-green-600 hover:bg-green-700 text-white py-4 md:py-6 rounded-xl flex justify-center items-center gap-3 font-bold shadow-lg text-xl md:text-2xl transition-all active:scale-95">
