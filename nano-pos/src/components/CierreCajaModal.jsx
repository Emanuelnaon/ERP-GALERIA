...
export default function CierreCajaModal({ turno, onClose, onCierreExitoso }) {
    ...
    useEffect(() => {
        const fetchGastos = async () => {
            const { data } = await supabase.from('gastos_caja').select('monto').eq('turno_id', turno.id);

            const sum = data?.reduce((acc, curr) => acc + Number(curr.monto), 0) || 0;
            setTotalGastos(sum);
        };
        fetchGastos();
    }, [turno.id]);
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm p-4">
            <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden text-white relative">
                ...
                <div className="bg-red-600/20 border-b border-red-500/30 p-6 text-center">
                    ...
                    <h2 className="text-2xl font-bold">Cierre de Caja (Arqueo)</h2>
                    <p className="text-gray-400 mt-1">Contá los billetes del cajón antes de cerrar</p>
                </div>
                ...
                <div className="p-6 bg-gray-800/50">
                    ...
                    <div className="space-y-3 mb-6 text-lg">
                        ...
                        <div className="flex justify-between font-bold text-xl border-t border-gray-700 pt-3">
                            ...
                            <span className="text-blue-400">${totalEsperado}</span>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleCerrarCaja}>
                        ...
                        {efectivoDeclarado !== '' && (
                            <div
                                ...
                                {diferencia < 0 ? <AlertTriangle size={24} /> : <CheckCircle size={24} />}
                                <div>
                                    ...
                                    {diferencia < 0 && (
                                        <p className="text-sm opacity-80">Revisá si diste mal un vuelto.</p>
                                    )}
                                </div>
                            </div>
                        )}
